...
/**
 * Contains the id of the packetData it represents and only in TCP connections does it specify the length
 */
public class Packet {
	long clientID = 0;
	...
	/**
	 * Reads as much data from the buffer as it can and writes into the Packet byte array
	 * <p>
	 * If there is enough data read then the PacketData type and expected length is also determined.
	 * However the packet may be filled with more data than expected so a call to {@link #writeExtraDataToPacket(Packet)}
	 * is needed to move the extra data.
	...
	public void writeExtraDataToPacket(Packet source) {
		...
		if (source.currentLength > source.expectedLength + source.headerLength) {
			...
			// Copy extra data from source to this packet (data in source does not need to be removed)
			System.arraycopy(source.payload, source.expectedLength + source.headerLength, this.payload, 0, this.currentLength);
